...
Rocket_Launcher::Rocket_Launcher(GameCore *game_core, uint32_t id, uint32_t player_id)
  ...
  if (!~tank_body_model_index) {
...

    {
      ...
      turret_vertices.push_back(
          {{0.15f, -0.15f}, {0.0f, 0.0f}, {0.7f, 0.7f, 0.7f, 1.0f}});
      turret_vertices.push_back(
          {{0.35f, -0.15f}, {0.0f, 0.0f}, {0.7f, 0.7f, 0.7f, 1.0f}});
      turret_vertices.push_back(
          {{0.15f, 1.0f}, {0.0f, 0.0f}, {0.7f, 0.7f, 0.7f, 1.0f}});
      turret_vertices.push_back(
          {{0.35f, 1.0f}, {0.0f, 0.0f}, {0.7f, 0.7f, 0.7f, 1.0f}});
    ...
    turret_vertices.push_back(
      ...
      turret_vertices.push_back(
          {{-0.35f, 1.0f}, {0.0f, 0.0f}, {0.7f, 0.7f, 0.7f, 1.0f}});
      turret_indices.push_back(precision + 1 + 8);
      turret_indices.push_back(precision + 1 + 9);
      turret_indices.push_back(precision + 1 + 10);
      turret_indices.push_back(precision + 1 + 9);
      turret_indices.push_back(precision + 1 + 10);
      turret_indices.push_back(precision + 1 + 11);
...
void Rocket_Launcher::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
    auto &input_data = player->GetInputData();
  ...
  if (fire_count_down_ == 0) {
    ...
    if (player) {
      ...
      if (input_data.mouse_button_down[GLFW_MOUSE_BUTTON_LEFT]) {
        auto velocity = Rotate(glm::vec2{0.0f, 14.0f}, turret_rotation_);
...
bool Rocket_Launcher::IsHit(glm::vec2 position) const {
...
const char *Rocket_Launcher::UnitName() const {
  return "Rocket Launcher";
}

const char *Rocket_Launcher::Author() const {
  return "Illuminum";
}
}  // namespace battle_game::unit
